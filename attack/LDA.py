# Simple LDA key recovery verification
from sage.all import *
mask = 2
slot_cnt = 2
window = 5
n = 32
key = [int(randrange(256)) for _ in range(16)]
print("generated secret key:", key)

sbox =  []

def trace(pt):
    res = [sbox[b ^ k] for b, k in zip(pt, key)]
    res += [randrange(256) for _ in range((slot_cnt-1)*16)]
    res_mask = []
    # for v in res:
    #     for _ in range(mask-1):
    #         a = randrange(256)
    #         res_mask.append(a)
    #         v ^= a
    #     res_mask.append(v)
    for v in res:
        for bit_pos in range(8):
            bit = (v >> bit_pos) & 1
            
            for _ in range(mask - 1):
                a = randrange(2)
                res_mask.append(a)
                bit ^= a
            
            res_mask.append(bit)

    # print("res (binary):", [format(r, '08b') for r in res])
    # print("res_mask (binary):", [format(m, '08b') for m in res_mask])
    return vector(GF(2), res_mask)


recovered_key = [None] * 16
t = []
pts = []

print("collecting", n, "traces")
for _ in range(n):
    pt = [int(randrange(256)) for _ in range(16)]
    pts.append(pt)
    t.append(trace(pt))

ones_column = vector(GF(2), [1] * n)

for idx in range(16):
    print("attacking", idx, "-th key")

    # slice window
    for k in range(256):
        if(recovered_key[idx] == None):
            # print(k)
            target = []
            for i in range(n):
                out = sbox[pts[i][idx] ^ k]
                target.append(out & 1)
            target = vector(GF(2), target)
            for startpos in range(slot_cnt*16*8 - window):
                mat = matrix(t[i][startpos : startpos + window] for i in range(n))
                mat = mat.augment(ones_column)
                if mat.nrows() == len(target):
                    try:
                        if mat.solve_right(target):
                            print("idx:", idx, "key byte:", k, "match")
                            recovered_key[idx] = k
                            break
                    except ValueError:
                        continue

print("recovered key", "".join("%02x" % c for c in recovered_key))
print("   actual key", "".join("%02x" % c for c in key))
if(key == recovered_key):
    print("successful match!")