...
cnt_pair = [[0 for _ in range(2 ** 8)] for _ in range(int(16 * 8 * mask * slot_cnt *5 * 0.18))]
cnt_k = [0] * (2 ** 8)
...
for exp_i in range(exp_cnt):
    ...
    if (exp_i+1) in pic:
        ...
        plt.xlabel('Window')
        plt.ylabel('Count')
        plt.savefig(f'pic/cnt_pair_w{window}_exp{exp_i+1}_3.pdf')
        plt.savefig(f'pic/cnt_pair_w{window}_exp{exp_i+1}_3.png')
